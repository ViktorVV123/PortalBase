...
export type UseMainCrudDeps = {
    ...
    setLastPrimary: Dispatch<SetStateAction<Record<string, unknown>>>;
...
export function useMainCrud({
    ...
    const cancelAdd = useCallback(() => { setIsAdding(false); setDraft({}); }, []);
    ...
    const cancelEdit = useCallback(() => { setEditingRowIdx(null); setEditDraft({}); setEditSaving(false); }, []);
    ...
    const submitEdit = useCallback(async () => {
        ...
        try {
            ...
            const url = `/data/${pf.formId}/${selectedWidget.id}`;
            try { await api.patch(url, body); }
            catch (err: any) {
                const status = err?.response?.status;
                const detail = err?.response?.data?.detail ?? err?.response?.data ?? err?.message;
                if (status === 404 && String(detail).includes('Update query not found')) return;
                if (status === 404) { await api.patch(`${url}/`, body); } else { throw err; }
            }
    ...
    const deleteRow = useCallback(async (rowIdx: number) => {
        ...
        try {
            ...
            try { await api.delete(url, { data: body }); }
            catch (err: any) {
                const status = err?.response?.status;
                const detail = err?.response?.data?.detail ?? err?.response?.data ?? err?.message;
                if (status === 404 && String(detail).includes('Delete query not found')) return;
                if (status === 404) { await api.delete(`${url}/`, { data: body }); } else { throw err; }
            }
    ...
    }, [selectedWidget, preflightDelete, formDisplay.data, pkToKey, reloadTree, activeFilters, setFormDisplay, lastPrimary, setSubDisplay, setSelectedKey]);
